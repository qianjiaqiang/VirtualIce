    ...
    try:
        ...
        if ivalue < 2 or ivalue >= 1000000:
            raise argparse.ArgumentTypeError(
                "Number of particles must be between 2 and 1000000 or 'max'"
            )
        return ivalue
    except ValueError:
        raise argparse.ArgumentTypeError(
            "Number of particles must be an integer between 2 and 1000000 or 'max'"
        )
...
def check_binning(value):
    ...
    try:
        if ivalue < 2 or ivalue >= 64:
            raise argparse.ArgumentTypeError(
                "Binning must be an integer between 2 and 64"
            )
        return ivalue
    except ValueError:
...
def format_structure_sets(structure_sets):
    ...
    for structure_set in structure_sets:
        ...
        if isinstance(structure_set[0], list):
            ...
            flat_set = [
                s[0] for s in structure_set
            ]  # Extract strings from the inner lists
...
def parse_structure_input(structure_input):
    ...
    for item in structure_input:
        if item.startswith("["):
            if in_bracket:
                raise argparse.ArgumentTypeError(
                    f"Nested brackets are not allowed: {item}"
                )
            in_bracket = True
            current_set = []
        ...
        if item.endswith("]"):
            if not in_bracket:
                raise argparse.ArgumentTypeError(
                    f"Closing bracket without opening bracket: {item}"
                )
            in_bracket = False
...
def parse_aggregation_amount(values, num_micrographs):
    ...
    def parse_single_value(value):
        ...
        if 0 <= numeric_value <= 10:
            ...
            print_and_log(
                f"Warning: Input value {value} is outside the allowed range (0-10). Value adjusted to the nearest valid value."
            )
            return [numeric_value]
    ...
    if values[-1].lower() in {"r", "random"}:
        ...
        if len(values) == 1:
            ...
            combined_values = [
                parse_single_value(random.choice(base_values))
                for _ in range(num_micrographs)
            ]
...
def convert_color_to_rgb(color):
    ...
    if color == "random" or color == "r":
        ...
        raise ValueError(
            "Color must be a valid color name, a list of 3 RGB values (0-255), or 'random'"
        )
...
\033[1mExamples:\033[0m
  ...
  4. Advanced usage: virtualice.py -s 1PMA -n 5 -om preferred -pw 0.9 -pa [*,90,0] [90 180 *] -aa l h r -ne --use_cpu -V 2 -3
    ...
    input_group.add_argument(
        ...
        "--image_directory",
        type=str,
        default=os.path.join(installation_directory, "ice_images"),
        help="Local directory name where the micrographs are stored in mrc format. They need to be accompanied with a text file containing image names and defoci (see --image_list_file). Default directory is %(default)s",
    )
    input_group.add_argument(
        "-i",
        "--image_list_file",
        ...
        default=os.path.join(
            installation_directory, "ice_images/good_images_with_defocus.txt"
        ),
        help="File containing local filenames of images with a defocus value after each filename (space between). Default is '%(default)s'.",
    )
    input_group.add_argument(
        "-me",
        "--max_emdb_size",
        type=float,
        default=512,
        help="The maximum allowed file size in megabytes. Default is %(default)s",
    )

    # Particle and Micrograph Generation Options
    particle_micrograph_group = parser.add_argument_group(
        "\033[1mParticle and Micrograph Generation Options\033[0m"
    )
    particle_micrograph_group.add_argument(
        ...
        type=int,
        default=5,
        help="Number of micrographs to create for each structure requested. Default is %(default)s",
    )
    particle_micrograph_group.add_argument(
        "-N",
        "--num_particles",
        type=check_num_particles,
        help="Number of particles to project onto the micrograph after rotation. Input an integer or 'max'. Default is a random number (weighted to favor numbers above 100 twice as much as below 100) up to a maximum of the number of particles that can fit into the micrograph without overlapping.",
    )
    particle_micrograph_group.add_argument(
        "-a",
        "--apix",
        type=float,
        default=1.096,
        help="Pixel size (in Angstroms) of the ice images, used to scale pdbs during pdb>mrc conversion (EMAN2 e2pdb2mrc.py option). Default is %(default)s (the pixel size of the ice images used during development)",
    )
    particle_micrograph_group.add_argument(
        "-r",
        "--pdb_to_mrc_resolution",
        type=float,
        default=3,
        help="Resolution in Angstroms for PDB to MRC conversion (EMAN2 e2pdb2mrc.py option). Default is %(default)s",
    )
    particle_micrograph_group.add_argument(
        "-st",
        "--std_threshold",
    ...
    particle_micrograph_group.add_argument(
        ...
        type=float,
        default=33.33,
        help="How much larger to make the resulting mrc file from the pdb file compared to the minimum equilateral cube. Extra space allows for more delocalized CTF information (default: %(default)s; ie. %(default)s%% larger)",
    )
    particle_micrograph_group.add_argument(
        "-D",
        "--distribution",
    ...
    particle_micrograph_group.add_argument(
        ...
        default="random",
        help="Specify whether to allow overlapping particles. Options are 'True', 'False', or 'random'. Default is %(default)s",
    )
    particle_micrograph_group.add_argument(
        "-ado",
        "--allowed_overlap",
        type=float,
        default=0.92,
        help="A factor related to how close non-overlapping particles can be when determining whether tosave their coordinates or not. Smaller = closer. Default is %(default)s, which was heuristically determined to work for globular proteins.",
    )
    particle_micrograph_group.add_argument(
        "-nl",
        "--num_particle_layers",
        type=int,
        default=2,
        help="If overlapping particles is allowed, this is the number of overlapping particle layers allowed (soft condition, not strict. Used in determining the maximum number of particles that can be placed in a micrograph). Default is %(default)s",
    )
    particle_micrograph_group.add_argument(
        "-so",
        "--save_overlapping_coords",
        action="store_true",
        help="Save overlapping particle coordinates to output files. Default is to not save overlapping particle",
    )
    particle_micrograph_group.add_argument(
        "-B",
        "--border",
        type=int,
        default=-1,
        help="Minimum distance of center of particles from the image border. Default is %(default)s = reverts to half boxsize",
    )
    particle_micrograph_group.add_argument(
        "-ne",
        "--no_edge_particles",
        action="store_true",
        help="Prevent particles from being placed up to the edge of the micrograph. By default, particles can be placed up to the edge.",
    )
    particle_micrograph_group.add_argument(
        "-se",
        "--save_edge_coordinates",
        action="store_true",
        help="Save particle coordinates that are closer than --border or closer than half a particle box size (if --border is not specified) from the edge. Requires --no_edge_particles to be False or --border to be greater than or equal to half the particle box size.",
    )
    ...
    simulation_group.add_argument(
        ...
        "--dose_damage",
        type=str,
        choices=["None", "Light", "Moderate", "Heavy", "Custom"],
        default="Moderate",
        help="Simulated protein damage due to accumulated dose, applied to simulated particle frames. Uses equation given by Grant & Grigorieff, 2015. Default is %(default)s",
    )
    simulation_group.add_argument(
        "-da",
        "--dose_a",
        type=float,
        required=False,
        help="Custom value for the 'a' variable in equation (3) of Grant & Grigorieff, 2015 (only required if '--dose-preset Custom' is chosen).",
    )
    simulation_group.add_argument(
        "-db",
        "--dose_b",
        type=float,
        required=False,
        help="Custom value for the 'b' variable in equation (3) of Grant & Grigorieff, 2015 (only required if '--dose-preset Custom' is chosen).",
    )
    simulation_group.add_argument(
        "-dc",
        "--dose_c",
        type=float,
        required=False,
        help="Custom value for the 'c' variable in equation (3) of Grant & Grigorieff, 2015 (only required if '--dose-preset Custom' is chosen).",
    )
    simulation_group.add_argument(
        "-nf",
        "--num_simulated_particle_frames",
        type=int,
        default=50,
        help="Number of simulated particle frames to generate Poisson noise and optionally apply dose damaging. Default is %(default)s",
    )
    simulation_group.add_argument(
        "-m",
        "--min_ice_thickness",
        type=float,
        default=30,
        help="Minimum ice thickness, which scales how much the particle is added to the image (this is a relative value). Default is %(default)s",
    )
    simulation_group.add_argument(
        "-M",
        "--max_ice_thickness",
        type=float,
        default=150,
        help="Maximum ice thickness, which scales how much the particle is added to the image (this is a relative value). Default is %(default)s",
    )
    simulation_group.add_argument(
        "-t",
        "--ice_thickness",
    ...
    simulation_group.add_argument(
        ...
        "--reorient_mrc",
        action="store_true",
        help="Reorient the MRC file (either provided as a .mrc file or requested from EMDB) so that the structure's principal axes align with the coordinate axes. Note: .pdb files are automatically reoriented, EMDB files are often too big to do so quickly. Default is %(default)s",
    )
    simulation_group.add_argument(
        "-om",
        "--orientation_mode",
        type=str,
        choices=["random", "uniform", "preferred"],
        default="random",
        help="Orientation mode for projections. Options are: random, uniform, preferred. Default is %(default)s",
    )
    simulation_group.add_argument(
        "-pa",
        "--preferred_angles",
        ...
        nargs="+",
        default=None,
        help="List of sets of three Euler angles (in degrees) for preferred orientations. Use '*' as a wildcard for random angles. Example: '[90,0,0]' or '[*,0,90]'. Euler angles are in the range [0, 360] for alpha and gamma, and [0, 180] for beta. Default is %(default)s",
    )
    simulation_group.add_argument(
        "-av",
        "--angle_variation",
        type=float,
        default=5.0,
        help="Standard deviation for normal distribution of variations around preferred angles (in degrees). Default is %(default)s",
    )
    simulation_group.add_argument(
        "-pw",
        "--preferred_weight",
        type=float,
        default=0.9,
        help="Weight of the preferred orientations in the range [0, 1] (only used if orientation_mode is preferred). Default is %(default)s",
    )
    simulation_group.add_argument(
        "-amp",
        "--ampcont",
        type=float,
        default=10,
        help="Amplitude contrast percentage when applying CTF to projections (EMAN2 CTF option). Default is %(default)s (ie. 10%%)",
    )
    simulation_group.add_argument(
        "-bf",
        "--bfactor",
        type=float,
        default=50,
        help="B-factor in A^2 when applying CTF to projections (EMAN2 CTF option). Default is %(default)s",
    )
    simulation_group.add_argument(
        "-cs",
        "--Cs",
        type=float,
        default=0.001,
        help="Microscope spherical aberration when applying CTF to projections (EMAN2 CTF option). Default is %(default)s because the microscope used to collect the provided buffer cryoEM micrographs has a Cs corrector",
    )
    simulation_group.add_argument(
        "-K",
        "--voltage",
        type=float,
        default=300,
        help="Microscope voltage (keV) when applying CTF to projections (EMAN2 CTF option). Default is %(default)s",
    )

    # Junk Labels Options
    junk_labels_group = parser.add_argument_group("\033[1mJunk Labels Options\033[0m")
    junk_labels_group.add_argument(
        "-nj",
        "--no_junk_filter",
        action="store_true",
        help="Turn off junk filtering; i.e. Do not remove particles from coordinate files that are on/near junk or substrate.",
    )
    junk_labels_group.add_argument(
        "-S",
        "--json_scale",
        type=int,
        default=4,
        help="Binning factor used when labeling junk to create the JSON files with AnyLabeling. Default is %(default)s",
    )
    junk_labels_group.add_argument(
        "-x",
        "--flip_x",
        action="store_true",
        help="Flip the polygons that identify junk along the x-axis",
    )
    junk_labels_group.add_argument(
        "-y",
        "--flip_y",
        action="store_true",
        help="Flip the polygons that identify junk along the y-axis",
    )
    junk_labels_group.add_argument(
        "-pe",
        ...
        type=int,
        default=5,
        help="Number of pixels to expand each polygon in the JSON file that defines areas to not place particle coordinates. The size of the pixels used here is the same size as the pixels that the JSON file uses (ie. the binning used when labeling the micrographs in AnyLabeling). Default is %(default)s",
    )

    # Particle Cropping Options
    particle_cropping_group = parser.add_argument_group(
        "\033[1mParticle Cropping Options\033[0m"
    )
    particle_cropping_group.add_argument(
        "-C",
        "--crop_particles",
        action="store_true",
        help="Enable cropping of particles from micrographs. Particles will be extracted to the [structure_name]/Particles/ directory as .mrc files. Default is no cropping.",
    )
    particle_cropping_group.add_argument(
        "-CM",
        "--max_crop_particles",
        type=int,
        default=None,
        help="Maximum number of particles to crop from micrographs.",
    )
    particle_cropping_group.add_argument(
        "-X",
        "--box_size",
        type=int,
        default=None,
        help="Box size for cropped particles (x and y dimensions are the same). Particles with box sizes that fall outside the micrograph will not be cropped. Default is the size of the mrc used for particle projection after internal preprocessing.",
    )
    ...
    output_group.add_argument(
        ...
        "--output_directory",
        type=str,
        help="Directory to save all output files. If not specified, a unique directory will be created.",
    )
    output_group.add_argument(
        "--mrc",
        action="store_true",
        default=True,
        help="Save micrographs as .mrc (default if no format is specified)",
    )
    output_group.add_argument(
        "-P", "--png", action="store_true", help="Save micrographs as .png"
    )
    output_group.add_argument(
        "-J", "--jpeg", action="store_true", help="Save micrographs as .jpeg"
    )
    output_group.add_argument(
        "-Q",
        "--jpeg_quality",
        type=int,
        default=95,
        help="Quality of saved .jpeg images (0 to 100). Default is %(default)s",
    )
    output_group.add_argument(
        "-b",
        "--binning",
        type=check_binning,
        default=1,
        help="Bin/Downsample the micrographs by Fourier cropping after superimposing particle projections. Binning is the sidelength divided by this factor (e.g. -b 4 for a 4k x 4k micrograph will result in a 1k x 1k micrograph) (e.g. -b 1 is unbinned). Default is %(default)s",
    )
    output_group.add_argument(
        "-k",
        "--keep",
        action="store_true",
        help="Keep the non-downsampled micrographs if downsampling is requested. Non-downsampled micrographs are deleted by default. Flag is currently forced to True until bug is fixed.",
    )
    output_group.add_argument(
        "-I",
        "--imod_coordinate_file",
        action="store_true",
        help="Also output one IMOD .mod coordinate file per micrograph. Note: IMOD must be installed and working",
    )
    output_group.add_argument(
        "-O",
        "--coord_coordinate_file",
        action="store_true",
        help="Also output one .coord coordinate file per micrograph",
    )
    output_group.add_argument(
        "-3",
        "--view_in_3dmod",
        action="store_true",
        help="View generated micrographs in 3dmod at the end of the run",
    )
    output_group.add_argument(
        "-3r",
        "--imod_circle_radius",
        type=int,
        default=10,
        help="Radius of the circle drawn in IMOD .mod files. Default is %(default)s",
    )
    output_group.add_argument(
        "-3t",
        "--imod_circle_thickness",
        type=int,
        default=2,
        help="Thickness of the circular line drawn in IMOD .mod files. Default is %(default)s",
    )
    output_group.add_argument(
        "-3c",
        "--imod_circle_color",
        nargs="+",
        default="green",
        help="Color of the circle drawn in IMOD .mod files. Can be a single color name (e.g. 'red', 'green', 'blue'), three RGB values (0-255), 'random'/'r' for a random color per structure, or a list of colors (one per structure) for multi-structure micrographs. Default is %(default)s",
    )
    ...
    misc_group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use CPU for processing instead of GPU. Default: Use GPUs if available",
    )
    misc_group.add_argument(
        "-g",
        "--gpus",
        ...
        nargs="+",
        default=None,
        help="Specify which GPUs to use by their IDs for various processing steps: micrograph downsampling. Default: Use all available GPUs",
    )
    misc_group.add_argument(
        "-ps",
        "--parallelize_structures",
        type=int,
        default=None,
        help="Maximum number of parallel processes for each structure requested. Default is the number of structures requested or one-fourth the number of CPU cores available, whichever is smaller",
    )
    misc_group.add_argument(
        "-pm",
        "--parallelize_micrographs",
        type=int,
        default=None,
        help="Number of parallel processes for generating each micrograph. Default is the number of micrographs requested or one-fourth the number of CPU cores available, whichever is smaller",
    )
    misc_group.add_argument(
        "-c",
        "--cpus",
        type=int,
        default=None,
        help="Number of CPUs to use for various processing steps: Adding Poisson noise to and dose damaging particle frames, generating particle projections, micrograph downsampling, and particle cropping. Default is the number of CPU cores available, minus the number of structures parallelized across minus the number of micrographs parallelized across",
    )
    misc_group.add_argument(
        "-V",
        "--verbosity",
        type=int,
        default=1,
        help="Set verbosity level: 0 (quiet), 1 (some output), 2 (verbose), 3 (debug). For 0-2, a log file will be additionally written with 2. For 3, a log file will be additionally written with 3. Default is %(default)s",
    )
    misc_group.add_argument(
        "-q",
        "--quiet",
        action="store_true",
        help="Set verbosity to 0 (quiet). Overrides --verbosity if both are provided",
    )
    misc_group.add_argument(
        "-v",
        "--version",
        ...
        version=f"VirtualIce v{__version__}",
    )
    ...
    if args.use_gpu:
        ...
        if not args.use_gpu:  # Check if get_gpu_ids returned nothing; fallback to CPUs
            ...
            args.gpu_ids = None
            print_and_log(
                "GPUs or CuPy not configured properly. Falling back to CPUs for processing."
            )
        else:
        ...
        args.gpu_ids = None
        print_and_log(
            "GPUs requested, but not available or CuPy not installed. Using only CPUs for processing."
        )
    else:
        ndimage = scipy_ndimage
        fftpack = scipy_fftpack
    ...
    flattened_structures = [
        structure for structure_set in args.structures for structure in structure_set
    ]
    ...
    if args.parallelize_structures is None:
        ...
        args.parallelize_structures = min(
            max(1, available_cpus // 4), len(flattened_structures)
        )
    else:
        args.parallelize_structures = min(
            args.parallelize_structures, len(flattened_structures)
        )

    if args.parallelize_micrographs is None:
        args.parallelize_micrographs = min(max(1, available_cpus // 4), args.num_images)
    else:
        args.parallelize_micrographs = min(
            args.parallelize_micrographs, args.num_images
        )

    if args.cpus is None:
        args.cpus = max(
            1,
            available_cpus - args.parallelize_structures - args.parallelize_micrographs,
        )
    ...
    args.distribution_mapped = distribution_mapping.get(
    ...
    if args.crop_particles and not args.mrc:
        args.mrc = True
        print_and_log(
            f"Notice: Since cropping (--crop_particles) is requested, then --mrc must be turned on. --mrc is now set to True."
        )

    if not (args.mrc or args.png or args.jpeg):
        parser.error(
            "No format specified for saving images. Please specify at least one format."
        )
    ...
    validate_positive_float(
        parser, "--pdb_to_mrc_resolution", args.pdb_to_mrc_resolution
    )
    ...
    validate_positive_int(
        parser, "--parallelize_structures", args.parallelize_structures
    )
    ...
    if args.dose_damage == "Custom":
        if args.dose_a is None or args.dose_b is None or args.dose_c is None:
            parser.error(
                "--dose_a, --dose_b, and --dose_c must be provided for --dose_damage=Custom."
            )
    ...
    if isinstance(args.imod_circle_color, list):
        if len(args.imod_circle_color) == 1:
            ...
            if color_value in ["r", "random"]:
                args.imod_circle_color = (
                    "random"  # Will be converted to random colors per structure later
                )
            else:
                args.imod_circle_color = (
                    color_value  # Single color name for all structures
                )
        elif all(len(rgb) == 3 for rgb in [args.imod_circle_color]) and all(
            str(v).isdigit() for v in args.imod_circle_color
        ):
            # Single RGB triplet provided
            args.imod_circle_color = [
                int(v) for v in args.imod_circle_color
            ]  # RGB values for all structures
        else:
            # Check if it's a list of color names or 'random'/'r'
            for color in args.imod_circle_color:
                if not (
                    isinstance(color, str)
                    and (color in ["r", "random"] or color in color_map.keys())
                ):
                    raise ValueError(
                        "Each color must be either a color name, 'random'/'r', or part of a single RGB triplet"
                    )
            # Keep the list of colors as is
    else:
        raise ValueError(
            "--imod_circle_color must be either a color name, exactly 3 RGB values, or 'random'/'r'"
        )
    ...
    argument_printout = textwrap.fill(
        formatted_output, width=80
    ...
    print_and_log(
        textwrap.fill(
            f"Generating {args.num_images} synthetic micrograph{'' if args.num_images == 1 else 's'} per structure set ({structure_sets_str}) using micrographs in {args.image_directory.rstrip('/')}/",
            width=80,
        ),
        logging.WARNING,
    )
    ...
    args.aggregation_amount = parse_aggregation_amount(
        args.aggregation_amount, args.num_images * len(flattened_structures)
    )
...
def setup_logging(script_start_time, verbosity):
    ...
    if verbosity < 3:
        ...
        file_handler.setLevel(
            logging.DEBUG
        )  # Set file logging to DEBUG for verbosity 3
...
def time_diff(time_diff):
    ...
    ):  # Show minutes if there are any hours or days
...
def is_local_mrc_path(input_str):
    ...
    return os.path.isfile(full_path) and (
        input_str.endswith(".mrc") or input_str.endswith(".map")
    )
...
def is_pdb_id(structure_input):
    ...
    return bool(
        re.match(r"^[0-9][A-Za-z0-9]{3}$", structure_input)
        and any(char.isalpha() for char in structure_input)
    )
    ...
    try:
        with request.urlopen(url) as response:
            if response.status == 200:
                ...
                sample_name = data.get("struct", {}).get(
...
def download_pdb(pdb_id, suppress_errors=False):
    ...
    try:  # Try downloading the regular .pdb file
        ...
        if not suppress_errors:
            print_and_log(
                f"Failed to download PDB {pdb_id}. HTTP Error: {e.code}\n",
                logging.WARNING,
            )
        return False
    except Exception as e:
        if not suppress_errors:
            print_and_log(
                f"An unexpected error occurred while downloading PDB {pdb_id}. Error: {e}\n",
                logging.WARNING,
            )
        return False
    ...
    while True:
        ...
        while not valid_pdb_id:
            ...
            pdb_id = "".join(
                ...
                + random.choices("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", k=3)
            )
        ...
        success = download_pdb(
            pdb_id, suppress_errors=True
        )  # Suppress errors for random PDB download attempts
        if success:
            return pdb_id
    ...
    try:
        ...
        with request.urlopen(req) as response:
            ...
            if file_size > max_emdb_size:
                if not suppress_errors:
                    print_and_log(
                        f"EMD-{emdb_id} file size ({file_size / (1024 * 1024):.2f} MB) exceeds the maximum allowed size ({max_emdb_size / (1024 * 1024):.2f} MB). Use the --max_emdb_size flag if you want to use this EMDB entry.",
                        logging.WARNING,
                    )
                return False
        ...
        print_and_log(
            f"[emd_{emdb_id}] Download and decompression complete for EMD-{emdb_id}."
        )
        ...
        if not suppress_errors:
            print_and_log(
                f"EMD-{emdb_id} not found. HTTP Error: {e.code}", logging.WARNING
            )
        return False
    except Exception as e:
        if not suppress_errors:
            print_and_log(
                f"An unexpected error occurred while downloading EMD-{emdb_id}. Error: {e}",
                logging.WARNING,
            )
        return False
...
def download_random_emdb(max_emdb_size):
    ...
    while True:
        ...
        emdb_id = str(
            random.randint(1, 45000)
        ).zfill(
            4
        )  # Makes a 4 or 5 digit number with leading zeros. Random 1-3 digits will also be 4 digit.
        success = download_emdb(emdb_id, max_emdb_size, suppress_errors=True)
        if success:
            return emdb_id


def process_structure_input(
    structure_input, max_emdb_size, std_devs_above_mean, pixelsize
):
    """
    ...
    if structure_input.lower() in ["r", "random"]:
        ...
        print_and_log(
            f"[{structure_name}] Using local PDB file: {structure_input}",
            logging.WARNING,
        )
        ...
        structure_name = os.path.basename(structure_input).split(".")[0]
        print_and_log(
            f"[{structure_name}] Using local MRC/MAP file: {structure_input}",
            logging.WARNING,
        )
        # Make a local copy of the file
        full_path = os.path.join(os.pardir, structure_input)
        ...
        if download_pdb(structure_input):
            print_and_log(
                f"[{structure_name}] Sample name: {get_pdb_sample_name(structure_input)}"
            )
            return (structure_input, "pdb")
        else:
            print_and_log(
                f"Failed to download PDB: {structure_input}. Please check the ID and try again.",
                logging.WARNING,
            )
            return None
    else:
        print_and_log(
            f"Unrecognized structure input: {structure_input}. Please enter a valid PDB ID, EMDB ID, local file path, or 'random'.",
            logging.WARNING,
        )
        return None
...
def normalize_and_convert_mrc(input_file):
    ...
    output_file = (
        ...
        else input_file.rsplit(".", 1)[0] + ".mrc"
    )
    ...
    try:
        ...
        with mrcfile.open(output_file, mode="r+") as mrc:
            data_padded = np.pad(
                mrc.data, pad_width, mode="constant", constant_values=np.mean(mrc.data)
            )
...
def scale_mrc_file(input_mrc_path, pixelsize):
    ...
    if scale_factor < 1:
        command = [
            ...
            "--clip={},{},{}".format(
                scaled_dimension_x, scaled_dimension_y, scaled_dimension_z
            ),
        ]
    elif scale_factor > 1:
        command = [
...
def convert_pdb_to_mrc(pdb_name, apix, res):
    ...
    try:
        ...
        mass = 0
        print_and_log(
            f"[{pdb_name}]Warning: Mass not found for PDB. Setting mass to 0.",
            logging.WARNING,
        )
    return mass
...
def reorient_mrc(input_mrc_path):
    ...
    aligned_coords = np.round(aligned_coords).astype(int) + np.round(
        mean_coords
    ).astype(int)
    ...
    for original_coord, new_coord in zip(non_zero_indices, aligned_coords):
        if (
            0 <= new_coord[0] < reoriented_data.shape[0]
            and 0 <= new_coord[1] < reoriented_data.shape[1]
            and 0 <= new_coord[2] < reoriented_data.shape[2]
        ):
            reoriented_data[new_coord[0], new_coord[1], new_coord[2]] = data[
                original_coord[0], original_coord[1], original_coord[2]
            ]
...
def lowPassFilter_gpu(imgarray_gpu, apix=1.0, bin=1, radius=0.0):
    ...
    filtered_imgarray_gpu = cp_gaussian_filter(
        imgarray_gpu, sigma=sigma / 10.0
    )  # 10.0 is a fudge factor that gets dose damaging about right.
...
def lowPassFilter(imgarray, apix=1.0, bin=1, radius=0.0):
    ...
    sitkImage = sitk.SmoothingRecursiveGaussian(
        sitkImage, sigma=sigma / 10.0
    )  # 10.0 is a fudge factor that gets dose damaging about right.
...
def write_all_coordinates_to_star(
    structure_name, image_path, particle_locations, orientations, defocus
):
    """
    ...
    with open(f"{structure_name}.star", "a") as star_file:
        for location, orientation in zip(particle_locations, orientations):
            ...
            star_file.write(
                f"{image_path} {x_shift} {y_shift} {alpha} {beta} {gamma} 1 {defocus} {defocus} 0\n"
            )


def convert_point_to_model(
    point_file,
    output_file,
    circle_radius,
    circle_thickness,
    ...
    try:
        ...
        print_and_log(
            "Error while converting coordinates using point2model.", logging.WARNING
        )
    except FileNotFoundError:
        print_and_log(
            "point2model not found. Ensure IMOD is installed and point2model is in your system's PATH.",
            logging.WARNING,
        )
...
def save_particle_coordinates(
    structure_name,
    particle_locations_with_orientations,
    output_path,
    micrograph_output_path,
    imod_coordinate_file,
    coord_coordinate_file,
    defocus,
    imod_circle_radius,
    ...
    write_all_coordinates_to_star(
        structure_name,
        micrograph_output_path + ".mrc",
        particle_locations,
        orientations,
        defocus,
    )
    ...
    if imod_coordinate_file:
        ...
        color = (
            imod_circle_color
            if isinstance(imod_circle_color, tuple)
            else convert_color_to_rgb(imod_circle_color)
        )
        write_mod_file(
            particle_locations,
            os.path.splitext(output_path)[0] + ".mod",
            imod_circle_radius,
            imod_circle_thickness,
            structure_name,
            color,
    ...
    if coord_coordinate_file:
        write_coord_file(
            particle_locations, os.path.splitext(output_path)[0] + ".coord"
        )
...
def estimate_mass_from_map(mrc_name):
    ...
    with mrcfile.open(f"{mrc_name}.mrc", mode="r") as mrc:
        ...
        protein_volume_angstroms_cubed = (
            np.sum(data > threshold) * voxel_volume_angstroms_cubed
        )
        protein_volume_cm_cubed = (
            protein_volume_angstroms_cubed * angstroms_cubed_to_cm_cubed
        )
...
def read_polygons_from_json(
    json_file_path, expansion_distance, flip_x=False, flip_y=False, expand=True
):
    """
        ...
        for shape in shapes:
            ...
            if expand:
                ...
                for point in polygon:
                    ...
                    unit_vector = vector / np.linalg.norm(
                        vector
                    )  # Normalize the vector
                    new_point = (
                        point + unit_vector * expansion_distance
                    )  # Move the point away from the centroid
...
def read_polygons_from_json_carbon(
    json_file_path, expansion_distance, flip_x=False, flip_y=False, expand=True
):
    """
    Modified by Jiaqiang Qian
    Read Carbon polygons from JSON file
    Read polygons from a JSON file generated by Anylabeling, optionally flip the coordinates,
    and optionally expand each polygon.
    ...
    with open(json_file_path, "r") as file:
        ...
        for shape in shapes:
            ...
            if polygon_type == "carbon":
                carbons.append(polygon.tolist())
            else:
                ices.append(polygon.tolist())

    return polygons, carbons, ices
...
def filter_coordinates_outside_polygons(particle_locations, json_scale, polygons):
    ...
    scaled_particle_locations = [
        (int(x / json_scale), int(y / json_scale)) for x, y in particle_locations
    ]
    ...
    for point in scaled_particle_locations:
        ...
        if not inside_any_polygon:
            ...
            filtered_particle_locations.append(
                (point[0] * json_scale, point[1] * json_scale)
            )

    return filtered_particle_locations


def filter_coordinates_inside_polygons(particle_locations, json_scale, polygons):
    """
    Filters out particle locations that are inside any polygon using OpenCV.
    ...
    """
    print_and_log("", logging.DEBUG)
    # An empty list to store particle locations that are inside the polygons
    filtered_particle_locations = []
    ...
    for point in scaled_particle_locations:
        ...
        # If the point is inside any polygon, add it to the filtered list
...
def next_divisible_by_primes(number, primes, count):
    ...
    for prime_combination in itertools.combinations(primes, count):
        ...
        next_multiple = (
            (number + least_common_multiple - 1) // least_common_multiple
...
def fourier_crop_gpu(image, downsample_factor):
    ...
    f_transform_cropped = f_transform_shifted[
...
def downsample_micrograph(image_path, downsample_factor, pixelsize, use_gpu):
    ...
    try:
        ...
        if ext == ".mrc":
            write_mrc(
                binned_image_path,
                downsampled_image.astype(np.float32),
                downsample_factor * pixelsize,
            )
        else:  # ext == .png/.jpeg
...
def parallel_downsample_micrographs(
    image_directory, downsample_factor, pixelsize, cpus, use_gpu, gpu_ids
):
    """
    ...
    if use_gpu:
        image_size = read_mrc(
            next(file for file in image_paths if file.endswith(".mrc"))
        ).nbytes
        batch_sizes = {}
        ...
        for gpu_id in gpu_ids:
            ...
            batch_sizes[gpu_id] = {
                ...
                "core_usage": utilization["core_usage"],
            }
            ...
            for gpu_id, batch_info in sorted_gpus:
                ...
                with cp.cuda.Device(gpu_id):
                    for image_path in batch_paths:
                        downsample_micrograph(
                            image_path, downsample_factor, pixelsize, use_gpu
                        )
                start = end
...
def downsample_star_file(input_star, output_star, downsample_factor):
    ...
    with open(input_star, "r") as infile, open(output_star, "w") as outfile:
        ...
        for line in infile:
            ...
            if (
                in_data_particles_block
                and line.strip()
                and not line.startswith(("data_", "loop_", "_"))
            ):
                parts = line.split()
                ...
                parts[1] = str(
                    float(parts[1]) / downsample_factor
                )  # Downsample x coordinate
                parts[2] = str(
                    float(parts[2]) / downsample_factor
                )  # Downsample y coordinate
...
def downsample_coordinate_files(
    structure_name,
    structure_set_name,
    binning,
    imod_coordinate_file,
    ...
    """
    print_and_log("", logging.DEBUG)
    downsample_star_file(
        f"{structure_name}.star", f"{structure_name}_bin{binning}.star", binning
    )
    if imod_coordinate_file:
        for filename in os.listdir(f"{structure_set_name}/"):
            if filename.endswith(".point"):
                ...
                output_point_file = os.path.join(
                    f"{structure_set_name}/bin_{binning}/",
                    filename.replace(".point", f"_bin{binning}.point"),
                )
    ...
    if coord_coordinate_file:
        for filename in os.listdir(f"{structure_set_name}/"):
            if filename.endswith(".coord"):
                input_file = os.path.join(f"{structure_set_name}/", filename)
                output_coord_file = os.path.join(
                    f"{structure_set_name}/bin_{binning}/",
                    filename.replace(".coord", f"_bin{binning}.coord"),
                )
...
def read_star_particles(star_file_path):
    ...
    with open(star_file_path, "r") as file:
        for i, line in enumerate(file):
            if "data_particles" in line:
                ...
                data_start_line = (
                    i + 14
                )  # Adjust if more lines are added to the star file
                break
    ...
    df = pd.read_csv(
        star_file_path,
        sep="\s+",
        skiprows=lambda x: x < data_start_line,
        header=None,
        names=[
            "micrograph_name",
            "coord_x",
...
def trim_vol_determine_particle_numbers(
    mrc_array,
    input_micrograph,
    scale_percent,
    allow_overlap,
    ...
    primes = [2, 3, 5]
    cube_size = (
        (min(next_divisible_by_primes(cube_size, primes, 2), 336) + 1) // 2
    ) * 2  # 336 is the largest practical box size before memory issues or seg faults
    ...
    print("input_micrograph.shape:", input_micrograph.shape)
    print("trimmed_mrc_array.shape:", trimmed_mrc_array.shape)
    print("mrc_array.shape:", mrc_array.shape)
    ...
    print("max_num_particles_without_overlap:", max_num_particles_without_overlap)
    ...
    if str(num_particles).lower() == "max":
        ...
        num_particles_to_project = non_uniform_random_number(
            2, max_num_particles, 100, 0.5
        )

    return trimmed_mrc_array, num_particles_to_project, max_num_particles


def not_trim_vol_determine_particle_numbers(
    mrc_array,
    input_micrograph,
    scale_percent,
    allow_overlap,
    ...
    """
    Dont't trim the volume, if needed ,resample it with a smaller cubic box in ChimeraX !
    Trim a volume and return a number of particles within a micrograph based on the maximum
    number of projections of this volume that can fit in the micrograph.
...
def determine_ice_and_particle_behavior(
    args,
    structure,
    structure_name,
    micrograph,
    ice_scaling_fudge_factor,
    remaining_aggregation_amounts,
    context,
):
    """
    ...
    structure, rand_num_particles, max_num_particles = (
        trim_vol_determine_particle_numbers(
            structure,
            micrograph,
            args.scale_percent,
            args.allow_overlap,
            args.num_particle_layers,
            args.num_particles,
        )
    )
    print("max_num_particles:", max_num_particles)
    print("args.num_particles:", args.num_particles)
    num_particles = (
        max_num_particles
        if str(args.num_particles).lower() == "max"
        else args.num_particles
        if args.num_particles
        and isinstance(args.num_particles, int)
        and args.num_particles <= max_num_particles
        else rand_num_particles
        if not args.num_particles
        else max_num_particles
    )

    if args.num_particles:
        print_and_log(
            f"{context} Attempting to find {int(num_particles)} particle locations for {structure_name} in the micrograph..."
        )
    else:
        print_and_log(
            f"{context} Choosing a random number of particles for {structure_name} to add to the micrograph..."
        )
    ...
    if args.ice_thickness is None:
        min_ice_thickness = ice_scaling_fudge_factor / (
            0.32 * args.max_ice_thickness + 20.45
        )
        max_ice_thickness = ice_scaling_fudge_factor / (
            0.32 * args.min_ice_thickness + 20.45
        )
    ...
    ice_thickness_printout = (ice_scaling_fudge_factor - 20.45 * ice_thickness) / (
        0.32 * ice_thickness
    )

    # Set the particle distribution type. Default to 'micrograph'
    non_random_distributions = {
        "micrograph",
        "gaussian",
        "circular",
        "inverse_circular",
    ...
    if args.distribution_mapped in non_random_distributions:
        ...
        non_random_dist_type = np.random.choice(
            ["circular", "inverse_circular", "gaussian", "micrograph"],
            p=[0.0025, 0.0075, 0.19, 0.8],
        )
    ...
    if dist_type == "non_random":
        if not args.num_particles:
            if non_random_dist_type == "circular":
                ...
                num_particles = max(
                    num_particles // 2, 2
                )  # Minimum number of particles is 2
    ...
    if args.aggregation_amount:
        ...
        aggregation_amount_index = random.randint(
            0, len(remaining_aggregation_amounts) - 1
        )
        aggregation_amount_val = remaining_aggregation_amounts.pop(
            aggregation_amount_index
        )
    else:
    ...
    return (
        ice_thickness,
        ice_thickness_printout,
        num_particles,
        dist_type,
        non_random_dist_type,
        aggregation_amount_val,
    )
...
def generate_orientations(preferred_angles, angle_variation, num_preferred, num_random):
    ...
    for _ in range(num_preferred):
        ...
        alpha = (
            np.round(np.random.uniform(0, 360), 3)
            if alpha == "*"
            else np.round(np.random.normal(float(alpha), angle_variation / 2), 3)
        )  # angle_variation / 2 because this is +-angle_variation
        beta = (
            np.round(np.random.uniform(0, 180), 3)
            if beta == "*"
            else np.round(np.random.normal(float(beta), angle_variation / 2), 3)
        )
        gamma = (
            np.round(np.random.uniform(0, 360), 3)
            if gamma == "*"
            else np.round(np.random.normal(float(gamma), angle_variation / 2), 3)
        )
...
def trim_volume(volume_data):
    ...
    trimmed_volume = volume_data[
        start_idx[0] : end_idx[0], start_idx[1] : end_idx[1], start_idx[2] : end_idx[2]
    ]
...
def pad_projection(projection, original_shape):
    ...
    padded_projection = np.pad(
        projection, ((pad_x, pad_x), (pad_y, pad_y)), mode="constant"
    )
...
def generate_projection_gpu(angle, volume_data_gpu, original_shape):
    ...
    rotated_volume_gpu = cp_affine_transform(
        volume_data_gpu,
        rotation_matrix,
        offset=center - cp.dot(rotation_matrix, center),
        order=1,
    )
...
def generate_projections(
    ...
    if orientation_mode == "random":
        ...
        orientations = [
            (angle, np.random.uniform(0, 180), np.random.uniform(0, 360))
            for angle in np.linspace(0, 360, num=num_projections, endpoint=False)
        ]
        ...
        orientations = generate_orientations(
            preferred_angles, angle_variation, num_preferred, num_random
        )
    ...
    if use_gpu:
            ...
            for gpu_id, batch_info in sorted_gpus:
                ...
                with cp.cuda.Device(gpu_id):
                    ...
                    for angle in projection_angles:
                        projection = generate_projection_gpu(
                            angle, trimmed_volume_gpu, structure.shape[:2]
                        )
        ...
        with ProcessPoolExecutor(max_workers=num_cores) as executor:
            futures = [
                executor.submit(generate_projection, angle, structure)
...
def generate_particle_locations(
    micrograph_image,
    image_size,
    ...
    if no_edge_particles:
        border_distances = [
            max(border_distance, hsw) for hsw in half_small_image_widths
        ]
    else:
        border_distances = [
            0 for _ in half_small_image_widths
        ]  # Minimum border distance is 0
    ...
    candidate_multiplier = (
        1.5 if not allow_overlap else 1.0
    )  # Generate more candidates if overlaps are not allowed
    ...
    if dist_type == "random":
        ...
        if non_random_dist_type == "circular":
            ...
            candidate_positions = np.column_stack(
                (x_coords.astype(int), y_coords.astype(int))
            )
        elif non_random_dist_type == "inverse_circular":
            ...
            distances = [
                np.hypot(exclusion_center[0] - x, exclusion_center[1] - y)
                for x, y in corners
            ]
            ...
            angles = np.random.uniform(0, 2 * np.pi, num_candidates)
            radii = np.sqrt(
                np.random.uniform(exclusion_radius**2, max_radius**2, num_candidates)
            )
            ...
            for _ in range(num_gaussians):
                ...
                candidate_positions.extend(
                    zip(x_coords.astype(int), y_coords.astype(int))
                )
            ...
            sigma = (
                min(width, height) / 1
            )  # Need to divide by something otherwise it breaks
            itk_image = sitk.GetImageFromArray(micrograph_image)
            filtered_micrograph_itk = sitk.SmoothingRecursiveGaussian(
                itk_image, sigma=sigma
            )
            ...
            inverse_normalized_prob_map = 0.8 + 0.4 * (
                1 - (prob_map - prob_map.min()) / (prob_map.max() - prob_map.min())
            )
        else:
            raise ValueError(
                f"Unknown non-random distribution type: {non_random_dist_type}"
            )
    else:
    ...
    for idx, (x, y) in enumerate(candidate_positions):
        ...
        if (
            x < min_border_dist
            or x >= width - min_border_dist
            or y < min_border_dist
            or y >= height - min_border_dist
        ):
            continue
    ...
    if not valid_candidates:
        print_and_log(
            "Warning: No valid candidate positions found for particles.",
            logging.WARNING,
        )
        # Return empty lists
        if dist_type == "non_random" and non_random_dist_type == "micrograph":
            return [
                [] for _ in num_particles_per_structure
            ], inverse_normalized_prob_map
        else:
    ...
    if allow_overlap:
        ...
        while (
            candidate_idx < len(valid_candidates)
            and sum(particle_counts) < total_required_particles
        ):
            for s_idx in structure_indices:
                if particle_counts[s_idx] < num_particles_per_structure[s_idx]:
                    ...
                    if (
                        x >= border_dist
                        and x < width - border_dist
                        and y >= border_dist
                        and y < height - border_dist
                    ):
        ...
        while (
            ...
            for s_idx in structure_indices:
                if particle_counts[s_idx] < num_particles_per_structure[s_idx]:
                    ...
                    if (
                        x < border_dist
                        or x >= width - border_dist
                        or y < border_dist
                        or y >= height - border_dist
                    ):
            ...
            candidate_idx += 1
    print("all_particle_locations:", len(all_particle_locations[0]))
    print("half_small_image_widths:", half_small_image_widths[0])
    if dist_type == "non_random" and non_random_dist_type == "micrograph":
...
def process_slices_gpu(args):
    ...
    for i in range(slice_gpu.shape[0]):
        ...
        for j in range(num_frames):
            ...
            if not (
                dose_a == dose_b == dose_c == 0
            ):  # All zero is equivalent to the user specifying None for dose_damage
                if j != dose_c:
                    lowpass = float(
                        cp.real(cp.complex64(dose_a / (j - dose_c)) ** (1 / dose_b))
                    )  # Equation (3) from Grant & Grigorieff, 2015. Assumes 1 e/A^2 per simulated frame.
                else:  # No divide by zero
                    lowpass = float(
                        cp.real(
                            cp.complex64(dose_a / (j - dose_c + 0.001)) ** (1 / dose_b)
                        )
    ...
    for i in range(slice.shape[0]):
        ...
        for j in range(num_frames):
            ...
            if not (
                ...
                if j != dose_c:
                    lowpass = float(
                        np.real(complex(dose_a / (j - dose_c)) ** (1 / dose_b))
                    )  # Equation (3) from Grant & Grigorieff, 2015. Assumes 1 e/A^2 per simulated frame.
                else:  # No divide by zero
                    lowpass = float(
                        np.real(complex(dose_a / (j - dose_c + 0.001)) ** (1 / dose_b))
                    )  # Slight perturbation of the equation above
...
def add_poisson_noise_gpu(
    ...
    while start < num_slices:
        ...
        for gpu_id, batch_info in sorted_gpus:
            ...
            with cp.cuda.Device(gpu_id):
                noisy_chunk = process_slices_gpu(
                    (
                        slice_chunk,
                        num_frames,
                        scaling_factor,
...
def add_poisson_noise(
    ...
    args = [
        (s, num_frames, scaling_factor, dose_a, dose_b, dose_c, apix) for s in slices
    ]
...
def apply_ctfs_with_eman2(
    particles, defocuses, ampcont, bfactor, apix, cs, voltage, num_workers
):
    """
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        results = list(
            executor.map(
                apply_ctf_with_eman2, particles, defocuses, [params] * len(particles)
            )
        )
...
def filter_out_overlapping_particles(
    particle_locations, half_small_image_width, allowed_overlap
):
    """
    ...
    print("overlapping_paticles:", overlapping_particles)
    # Filter out the overlapping particles
    filtered_locations = [
        loc
        for i, loc in enumerate(particle_locations)
        if i not in overlapping_particles
    ]
...
def create_collage(large_image, small_images, particle_locations, gaussian_variance):
    ...
    for i, small_image in enumerate(small_images):
        ...
        if collage_region.shape == small_image_region.shape:
            ...
            collage[y_start : y_start + min_height, x_start : x_start + min_width] += (
                small_image_region[:min_height, :min_width]
            )

    # Apply Gaussian noise across the entire collage to simulate the camera noise
    gaussian_noise = np.random.normal(
        loc=0, scale=np.sqrt(gaussian_variance), size=collage.shape
    )
...
def blend_images(
    ...
    all_small_images = input_options[
        "small_images"
    ]  # A list of small images for all structures
    all_particle_locations = input_options[
        "particle_locations"
    ]  # A list of particle locations for all structures
    all_orientations = input_options[
        "orientations"
    ]  # A list of orientations for all structures
    ...
    # deal with carbon
    carbon = junk_labels_options["carbon"]
    ...
    for i, structure_name in enumerate(structure_names):
        ...
        if not no_junk_filter:
            if os.path.exists(json_file_path) and not carbon:
                polygons = read_polygons_from_json(
                    json_file_path,
                    polygon_expansion_distance,
                    flip_x,
                    flip_y,
                ...
                num_particles_removed = len(particle_locations) - len(
                    filtered_particle_locations
                )
                print_and_log(
                    f"{context} {num_particles_removed} obscured particle{'' if num_particles_removed == 1 else 's'} removed for {structure_name} based on the JSON file."
                )
            # save particles inside carbon polygons
            elif os.path.exists(json_file_path) and carbon:
                polygons, carbon_polygons, ice_polygons = (
                    read_polygons_from_json_carbon(
                        json_file_path,
                        polygon_expansion_distance,
                        flip_x,
                        flip_y,
                ...
                num_particles_carbon = len(filtered_carbon_particle_locations)

                print_and_log(
                    f"{context} {num_particles_removed} obscured particle{'' if num_particles_removed == 1 else 's'} removed for {structure_name} based on the JSON file."
                )
                print_and_log(
                    f"{context} {num_particles_carbon} carbon particle{'' if num_particles_removed == 1 else 's'} found for {structure_name} based on the JSON file."
                )
            else:
                print_and_log(
                    f"{context} No JSON file found for bad micrograph areas: {json_file_path}",
                    logging.WARNING,
                )
                filtered_particle_locations = particle_locations
        else:
            print_and_log(
                f"{context} Skipping junk filtering for {structure_name} (i.e., not using JSON file)"
            )
            filtered_particle_locations = particle_locations

        filtered_all_particle_locations.append(filtered_particle_locations)

    def edge_particle_filtering(_before_filtered):
        _filtered = []
        for _i, _p_locations in enumerate(_before_filtered):
            _remained = _p_locations[:]
            _structure_name = structure_names[_i]
            _half_small_image_width = input_options["half_small_image_widths"][_i]

            if not particle_and_micrograph_generation_options["save_edge_coordinates"]:
                for _x, _y in _p_locations:
                    _reduced_sidelength = int(
                        np.ceil(
                            _half_small_image_width
                            * 100
                            / (
                                100
                                + particle_and_micrograph_generation_options[
    ...
    final_particle_locations = edge_particle_filtering(filtered_all_particle_locations)
    ...
    #        i
    #    ]  # Handle different sizes for each structure

    #    if not particle_and_micrograph_generation_options["save_edge_coordinates"]:
    #        for x, y in filtered_particle_locations:
    #            reduced_sidelength = int(
    #                np.ceil(
    #                    half_small_image_width
    #                    * 100
    #                    / (
    #                        100
    #                        + particle_and_micrograph_generation_options[
    ...
    #        num_particles_removed = len(filtered_particle_locations) - len(
    #            remaining_particle_locations
    #        )
    #        if num_particles_removed > 0:
    #            print_and_log(
    #                f"{context} {num_particles_removed} edge particle{'' if num_particles_removed == 1 else 's'} removed for {structure_name}."
    ...
    def overlapping_particle_filtering(_before_filtered):
        _filtered = []
        for _i, _particle_locations in enumerate(_before_filtered):
            if not particle_and_micrograph_generation_options[
                "save_overlapping_coords"
            ]:
                _half_small_image_width = input_options["half_small_image_widths"][_i]
                _filtered_particle_locations = filter_out_overlapping_particles(
                    _particle_locations, _half_small_image_width, allowed_overlap
                )
                num_particles_removed = len(_particle_locations) - len(
                    _filtered_particle_locations
                )
                if num_particles_removed > 0:
                    print_and_log(
                        f"{context} {num_particles_removed} overlapping particle{'' if num_particles_removed == 1 else 's'} removed for {structure_names[i]}."
                    )
                else:
                    print_and_log(
                        f"{context} 0 overlapping particles removed for {structure_names[i]}."
                    )
            else:
                _filtered_particle_locations = _particle_locations

            _filtered.append(_filtered_particle_locations)
    ...
    for i, structure_name in enumerate(structure_names):
        structure_particle_locations_with_orientations = [
            (loc, ori)
            for loc, ori in zip(
                final_filtered_particle_locations[i], all_orientations[i]
            )
        ]
...
def add_images(
    ...
    if output_options["save_as_mrc"]:
        print_and_log(
            f"{context} Writing synthetic micrograph: {output_file_path}.mrc..."
        )
        write_mrc(
            f"{output_file_path}.mrc",
            (result_image - np.mean(result_image)) / np.std(result_image),
            pixelsize,
        )  # Write normalized mrc (mean = 0, std = 1)
    ...
    if output_options["save_as_png"]:
        ...
        print_and_log(
            f"{context} Writing synthetic micrograph: {output_file_path}.png..."
        )
    ...
    if output_options["save_as_jpeg"]:
        ...
        result_image = result_image / result_image.max() * 255.0
        print_and_log(
            f"{context} Writing synthetic micrograph: {output_file_path}.jpeg..."
        )
        cv2.imwrite(
            f"{output_file_path}.jpeg",
            np.flip(result_image, axis=0),
            [cv2.IMWRITE_JPEG_QUALITY, output_options["jpeg_quality"]],
        )
    ...
    with cp.cuda.Device(gpu_id):
        with mrcfile.open(micrograph_path, permissive=True) as mrc:
            ...
            for _, row in particle_rows.iterrows():
                ...
                if (
                    x - half_box_size < 0
                    or y - half_box_size < 0
                    or x + half_box_size > micrograph_data.shape[1]
                    or y + half_box_size > micrograph_data.shape[0]
                ):
                    continue

                cropped_particle = micrograph_data[
                    y - half_box_size : y + half_box_size,
                    x - half_box_size : x + half_box_size,
                ]
                particle_path = os.path.join(
                    particles_dir, f"particle_{row['particle_counter']:010d}.mrc"
                )
                write_mrc(
                    particle_path,
                    cp.asnumpy(cropped_particle).astype(np.float32),
                    pixelsize,
                )
                cropped_count += 1
    ...
    with mrcfile.open(micrograph_path, permissive=True) as mrc:
        for _, row in particle_rows.iterrows():
            ...
            if (
                x - half_box_size < 0
                or y - half_box_size < 0
                or x + half_box_size > mrc.data.shape[1]
                or y + half_box_size > mrc.data.shape[0]
            ):
                continue

            cropped_particle = mrc.data[
                y - half_box_size : y + half_box_size,
                x - half_box_size : x + half_box_size,
            ]
            particle_path = os.path.join(
...
def crop_particles_from_micrographs(
    structure_name,
    structure_set_name,
    box_size,
    pixelsize,
    ...
    structure_name = (
        structure_name
        if structure_name.isalnum()
        else structure_name.split("/")[-1].split("\\")[-1].rsplit(".", 1)[0]
    )  # Strips file extension if the user inputted a local file
    ...
    star_file_path = next(
        (
            path
            for path in [
                f"{structure_set_name}/{structure_name}.star",
                f"{structure_name}.star",
            ]
            if os.path.isfile(path)
        ),
        None,
    )  # Hack to get around a bug where the star file is either in the base directory or structure directory.
    ...
    if use_gpu and gpu_ids:
        ...
        for gpu_id in gpu_ids:
            ...
            batch_size = get_max_batch_size(
                box_size * box_size * 4, utilization["free_mem"]
            )  # * 4 is because each pixel takes 4 bytes for float32
            batch_sizes[gpu_id] = {
                "batch_size": batch_size,
                "core_usage": utilization["core_usage"],
            }
                ...
                with cp.cuda.Device(gpu_id):
                    ...
                    for micrograph_name in batch_micrographs:
                        ...
                        if not os.path.exists(micrograph_path):
                            print_and_log(
                                f"{context} Micrograph not found: {micrograph_path}",
                                logging.WARNING,
                            )
                            continue
                        print_and_log(
                            f"{context} Extracting {len(particle_rows)} {structure_name} particles for {micrograph_name}...",
                            logging.DEBUG,
                        )
                        batch_paths.append(micrograph_path)
                        batch_particle_rows.append(particle_rows)
                    total_cropped += crop_particles_gpu_batch(
                        batch_paths,
                        batch_particle_rows,
                        particles_dir,
                        box_size,
...
def prepare_single_micrograph(args, structures):
    ...
    for idx, (structure_name, structure, mass, ice_scaling_fudge_factor) in enumerate(
        structures
    ):
        ...
        # print(structure_name, type(orientations), type(orientations[0]),orientations[0])
        # print(type(particles),type(particles[0]),particles.shape)
        ...
        if args.use_gpu == 0:
            ...
            noisy_particles = add_poisson_noise(
                particles,
                args.num_simulated_particle_frames,
                args.dose_a,
                args.dose_b,
...
def prepare_single_micrograph_relion(args, structures):
    """
    Prepare projections and noisy projections for structures
    """
    # assert len(structures)==1
    # structure = structures[0]
    print_and_log("Preparing projections and noisy projections for structures")
    all_structure_particles = []
    all_structure_particles_orientations = []
    all_structure_particles_noisy = []
    for idx, (structure_name, structure, mass, ice_scaling_fudge_factor) in enumerate(
        ...
        relion_projections = mrcfile.open(
            "../" + structure_name + ".mrcs", permissive=True
        )
        ...
        relion_projections_orientations = relion_projections_star[0].loc[
            :, ["_rlnAngleRot", "_rlnAngleTilt", "_rlnAnglePsi"]
        ]
        orientations = []
        for i, row in relion_projections_orientations.iterrows():
            orientations.append((row._rlnAngleRot, row._rlnAngleTilt, row._rlnAnglePsi))
...
def process_single_micrograph_with_projections(
    args,
    structures,
    line,
    total_structures,
    structure_index,
    micrograph_usage_count,
    remaining_aggregation_amounts,
    micrograph_number,
    structure_set_name,
    projected_particles,
    ...
    repeat_suffix = (
        f"_{micrograph_usage_count[fname]}" if micrograph_usage_count[fname] > 1 else ""
    )
    ...
    print_and_log(
        f"Generating synthetic micrograph #{micrograph_number} for SS #{structure_index + 1} ({structure_index + 1}/{total_structures}) from {fname}...",
        logging.WARNING,
    )
    ...
    print_and_log(
        f"{context} {'Allowing' if args.allow_overlap else 'Not allowing'} overlapping particles for this micrograph."
    )
    ...
    for structure_name, structure, mass, ice_scaling_fudge_factor in structures:
        ...
        (
            ...
            non_random_dist_type,
            aggregation_amount_val,
        ) = determine_ice_and_particle_behavior(
            args,
            structure,
            structure_name,
            micrograph,
        ...
        num_particles = (
            num_particles // len(structures)
        )  # Evenly divide the total number of particles amongst the structures in the set
        print_and_log(
            f"{context} {num_particles} particles of {structure_name} will be added to the micrograph"
        )
    ...
    for idx, (structure_name, structure, mass, ice_scaling_fudge_factor) in enumerate(
        ...
        print_and_log(
            f"{context} Skip Projecting the structure volume ({structure_name}) {num_particles_per_structure[idx]} times..."
        )
        projections_idx = np.random.choice(
            len(projected_orientations[idx]),
            size=len(particle_locations[idx]),
            replace=True,
        )
        ...
        all_particle_locations.append(
            particle_locations[idx]
        )  # Keep particle locations separated for each structure
        ...
        print_and_log(
            f"{context} Skip Simulating pixel-level Poisson noise{f' and dose damage' if args.dose_damage != 'None' else ''} across {args.num_simulated_particle_frames} particle frame{'s' if args.num_simulated_particle_frames != 1 else ''} for {structure_name}..."
        )
        ...
        print_and_log(
            f"{context} Applying CTF to {structure_name} based on defocus ({float(defocus):.4f} microns) and microscope parameters ({args.voltage} keV, AmpCont: {args.ampcont}%, Cs: {args.Cs} mm, Pixelsize: {args.apix} Angstroms) of the ice micrograph..."
        )
    ...
    print_and_log(
        f"{context} Adding {len(all_particles) * num_particles} particles to the micrograph{f' {dist_type}ly ({non_random_dist_type})' if dist_type == 'non_random' else f' {dist_type}ly' if dist_type else ''}{f' with aggregation amount of {aggregation_amount_val:.1f}' if args.distribution in ('m','micrograph') else ''} while adding Gaussian (white) noise and simulating a average relative ice thickness of {ice_thickness_printout:.1f} nm..."
    )
    input_options = {
        ...
        "pixelsize": args.apix,
        "structure_names": [
            structure[0] for structure in structures
    ...
    # add carbon options to junk_labels_options
    junk_labels_options = {
    ...
    output_options = {
        ...
        "output_paths": [
            f"{structure_set_name}/{fname}_{structure[0]}{repeat_suffix}"
            for structure in structures
        ],
        "imod_circle_radius": args.imod_circle_radius,
        "imod_circle_thickness": args.imod_circle_thickness,
        "imod_circle_color": [
            convert_color_to_rgb("random")
            if args.imod_circle_color == "random"
            else convert_color_to_rgb(
                args.imod_circle_color[i]
                if isinstance(args.imod_circle_color, list)
                and len(args.imod_circle_color) == len(structures)
                else args.imod_circle_color
            )
    ...
    for idx, structure in enumerate(structures):
        ...
        if idx == num_structures - 1:
            structure_projected = num_particles_projected - (
                particles_per_structure * (num_structures - 1)
            )
        else:
...
def process_single_micrograph(
    ...
    for idx, (structure_name, structure, mass, ice_scaling_fudge_factor) in enumerate(
        ...
        print_and_log(
            f"{context} Projecting the structure volume ({structure_name}) {num_particles_per_structure[idx]} times..."
        )
        ...
        # Step 4: Simulate noise, damage, and apply CTF to this structure's particles
        print_and_log(
            f"{context} Simulating pixel-level Poisson noise{f' and dose damage' if args.dose_damage != 'None' else ''} across {args.num_simulated_particle_frames} particle frame{'s' if args.num_simulated_particle_frames != 1 else ''} for {structure_name}..."
        )
        if args.use_gpu == 0:
            noisy_particles = add_poisson_noise_gpu(
                particles,
...
def process_single_structure(sub_structure_input, args):
    ...
    """
    result = process_structure_input(
        sub_structure_input, args.max_emdb_size, args.std_threshold, args.apix
    )
    if result:
        structure_name, structure_type = result
        if structure_type == "pdb":
            mass = convert_pdb_to_mrc(
                structure_name, args.apix, args.pdb_to_mrc_resolution
            )
            ...
            if args.reorient_mrc:
                structure = reorient_mrc(
                    f"{structure_name}.mrc"
...
def generate_micrographs(
    args, structure_set, structure_set_index, total_structure_sets
):
    """
    ...
    with ProcessPoolExecutor(max_workers=args.cpus) as executor:
        for structure_input in structure_set:
            if isinstance(structure_input, list):
                ...
                for sub_structure_input in structure_input:
                    tasks.append(
                        executor.submit(
                            process_single_structure, sub_structure_input, args
                        )
                    )
            else:
                # Handle single structure per micrograph
                tasks.append(
                    executor.submit(process_single_structure, structure_input, args)
                )
        ...
        for future in tasks:
            ...
            if result:
                ...
                structures.append(
                    (structure_name, structure, mass, ice_scaling_fudge_factor)
                )
                box_sizes.append(
                    structure.shape[0]
                )  # Store box size for this structure
    ...
    def format_structure_list(structure_set):
        ...
        for structure_input in structure_set:
            if isinstance(
                structure_input, list
            ):  # Handle multiple structures per micrograph (list of lists)
                formatted_list.extend(
                    [str(sub_structure) for sub_structure in structure_input]
                )
    ...
    print_and_log(
        f"{context} Selecting {args.num_images} random ice micrographs for Structure Set (SS): {', '.join(formatted_structure_list)}..."
    )
    selected_images = extend_and_shuffle_image_list(
        args.num_images, args.image_list_file
    )
    ...
    if args.parallelize_micrographs > 1:
        ...
        with ProcessPoolExecutor(max_workers=args.parallelize_micrographs) as executor:
            ...
            for i, line in enumerate(selected_images):
                ...
                # )
                future = executor.submit(
                    process_single_micrograph_with_projections,
                    args,
                    structures,
                    line,
                    total_structure_sets,
                    structure_set_index,
                    micrograph_usage_count,
                    remaining_aggregation_amounts,
                    micrograph_number,
    ...
    if args.binning > 1:
        print_and_log(
            f"{context} Binning/Downsampling micrographs by {args.binning} by Fourier cropping..."
        )
        parallel_downsample_micrographs(
            f"{structure_set_name}/",
            args.binning,
            args.apix,
            args.cpus,
            args.use_gpu,
        ...
        for structure_name, _, _, _ in structures:
            downsample_coordinate_files(
                structure_name,
                structure_set_name,
                args.binning,
                args.imod_coordinate_file,
                args.coord_coordinate_file,
                args.imod_circle_radius,
                args.imod_circle_thickness,
                args.imod_circle_color,
            )
    else:
        for structure_name, _, _, _ in structures:
    ...
    with open(f"virtualice_{args.script_start_time}_info.txt", "a") as f:
        if f.tell() == 0:  # Only write the header line if the file is new
            f.write(
                "structure_set_name structure_name mass(kDa) num_images num_particles_projected num_particles_saved\n"
            )
        for structure_name, _, mass, _ in structures:
            # Write the correct number of saved particles for each structure
            f.write(
                f"{structure_set_name} {structure_name} {mass} {args.num_images} {structure_particles_projected[structure_name]} {structure_particles_saved[structure_name]}\n"
            )
    ...
    if not args.keep:
        if args.binning > 1:
            ...
            for file in os.listdir(structure_set_name):
                ...
                if file.endswith((".mrc", ".coord", ".mod")) and not file.endswith(
                    f"_bin{args.binning}.mrc"
                ):
            ...
            for file_path in glob.glob(star_file):
                try:
                    ...
                    print_and_log(f"Error removing {file_path}: {e}", logging.WARNING)
...
def print_run_information(
    ...
    structure_sets,
    time_str,
    total_number_of_particles_projected,
    total_number_of_particles_with_saved_coordinates,
    total_cropped_particles,
    crop_particles,
    imod_coordinate_file,
    coord_coordinate_file,
    binning,
    ...
    print_and_log(
        f"Time to generate \033[1m{num_micrographs}\033[0m micrograph{'s' if num_micrographs != 1 else ''} "
    ...
    print_and_log(
        f"Total: \033[1m{total_number_of_particles_projected}\033[0m particles projected, "
        f"\033[1m{total_number_of_particles_with_saved_coordinates}\033[0m saved to coordinate files"
        + (
            f", \033[1m{total_cropped_particles}\033[0m particles cropped"
            if crop_particles
            else ""
        ),
        logging.WARNING,
    ...
    for structure_set_name, structure_set in zip(structure_set_names, structure_sets):
        ...
        structure_names = ", ".join(
            [s[0] if isinstance(s, list) else s for s in structure_set]
        )
        print_and_log(
            f"Structure set: \033[1m{structure_set_name} ({structure_names})\033[0m",
            logging.WARNING,
        )
    ...
    if binning > 1:
        print_and_log(
            f"  - Binned by {binning} data."
            if not keep
            else f"  - Binned data in bin_{binning} sub-director{'y' if total_structure_sets == 1 else 'ies'}.",
            logging.WARNING,
        )

    # STAR file information
    print_and_log(
        "  - One .star file per structure is in the SS sub-directory.", logging.WARNING
    )

    # COORD file information
    if coord_coordinate_file:
        print_and_log(
            "  - One (x, y) .coord file per micrograph is in the SS sub-directory.",
            logging.WARNING,
        )
    ...
    if imod_coordinate_file:
        print_and_log(
            "  - One IMOD .mod file per micrograph is in the SS sub-directory.",
            logging.WARNING,
        )
        print_and_log(
            "    To view, run a command like this: \033[1m3dmod image.mrc image.mod\033[0m  (Replace with your files)",
            logging.WARNING,
        )
...
def find_micrograph_files(structure_dirs):
    ...
    micrograph_files = sorted(
        micrograph_files,
        key=lambda f: (
            f.rsplit(".", 1)[0],
            extension_order.get("." + f.rsplit(".", 1)[1], 3),
        ),
    )
...
def view_in_3dmod_async(micrograph_files, imod_coordinate_file, structure_names):
    ...
    if len(micrograph_files) == 1 and imod_coordinate_file:
        ...
        first_structure = (
            structure_names[0][0]
            if isinstance(structure_names[0], list)
            else structure_names[0]
        )
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=args.parallelize_structures) as executor:
        for structure_set_index, structure_set in enumerate(args.structures):
            ...
            tasks.append(
                (task, structure_set)
            )  # Store task and associated structure set
        ...
        for task, structure_set in tasks:
            ...
            structure_particles_projected, structure_particles_saved, box_sizes = (
                task.result()
            )
            ...
            # Aggregate totals
            total_number_of_particles_projected += sum(
                structure_particles_projected.values()
            )
            total_number_of_particles_with_saved_coordinates += sum(
                structure_particles_saved.values()
            )
    ...
    if args.view_in_3dmod:
        ...
        if micrograph_files:
            threading.Thread(
                target=view_in_3dmod_async,
                args=(micrograph_files, args.imod_coordinate_file, args.structures[0]),
            ).start()
    ...
    if args.crop_particles:
        with ProcessPoolExecutor(max_workers=args.parallelize_structures) as executor:
            crop_tasks = []
            for structure_set_name, box_sizes in zip(
                structure_set_names, structure_set_box_sizes
            ):
                for i, structure_name in enumerate(
                    args.structures[structure_set_names.index(structure_set_name)]
                ):
                    # Use the appropriate box_size for each structure
                    box_size = (
                        args.box_size if args.box_size is not None else box_sizes[i]
                    )
                    structure_name_str = (
                        structure_name[0]
                        if isinstance(structure_name, list)
                        else structure_name
                    )  # Converts element of structure set to structure name, if necessary
                    crop_task = executor.submit(
                        crop_particles_from_micrographs,
                        structure_name_str,
                        structure_set_name,
                        box_size,
                        args.apix,
                        args.max_crop_particles,
                        args.cpus,
                        args.use_gpu,
                        args.gpu_ids,
                    )
    ...
    for structure_set_name, structure_set in zip(structure_set_names, args.structures):
        ...
        structure_names = [
            structure[0] if isinstance(structure, list) else structure
