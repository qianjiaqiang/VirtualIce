...
def read_polygons_from_json_carbon(json_file_path, expansion_distance, flip_x=False, flip_y=False, expand=True):
    """
...
def trim_vol_determine_particle_numbers(mrc_array, input_micrograph, scale_percent, allow_overlap, num_particle_layers, num_particles):
    ...
    max_num_particles_without_overlap = int(1 * input_micrograph.shape[0] * input_micrograph.shape[1] / (trimmed_mrc_array.shape[0] * trimmed_mrc_array.shape[1]) / (1 - scale_percent/100))
...
def not_trim_vol_determine_particle_numbers(mrc_array, input_micrograph, scale_percent, allow_overlap, num_particle_layers, num_particles):
    """
    ...
    # Set the maximum number of particle projections that can fit in the image
    max_num_particles_without_overlap = int(2 * input_micrograph.shape[0] * input_micrograph.shape[1] / (trimmed_mrc_array.shape[0] * trimmed_mrc_array.shape[1]) / (1.0 + scale_percent/100))
            ...
            for s_idx in structure_indices:
                if particle_counts[s_idx] < num_particles_per_structure[s_idx]:
                    ...
                    if placed_positions_list[s_idx]:
                        ...
                        min_distance = half_small_image_widths[s_idx] * 2.0  # Adjust the factor as needed
...
def prepare_single_micrograph_relion(args, structures):
    ...
    all_structure_particles_noisy = []
    for idx, (structure_name, structure, mass, ice_scaling_fudge_factor) in enumerate(structures):
        relion_projections=mrcfile.open("../"+structure_name+".mrcs",permissive=True)
        read_in_pixelSize = relion_projections.header["cella"]["x"]/ np.float32(relion_projections.header["nx"])
        print("pixelSize:", read_in_pixelSize)
        print("structure:",structure.shape)
        import torch
        import torchvision.transforms.v2 as v2
        tr = v2.Resize((structure.shape[0],structure.shape[0]))
        relion_projections_star = fileparser.getparticles("../"+structure_name+".star")
        ...
        relion_projections_orientations = relion_projections_star[0].loc[:,["_rlnAngleRot","_rlnAngleTilt","_rlnAnglePsi"]]
        orientations = []
...
def process_single_micrograph_with_projections(args, structures, line, total_structures, structure_index,
    ...
    for idx, (structure_name, structure, mass, ice_scaling_fudge_factor) in enumerate(structures):
        ...
        projections_idx = np.random.choice(len(projected_orientations[idx]), size=len(particle_locations[idx]), replace=True)
...
def generate_micrographs(args, structure_set, structure_set_index, total_structure_sets):
    ...
    projected_particles, projected_orientations, projected_noisy = prepare_single_micrograph_relion(args, structures)
